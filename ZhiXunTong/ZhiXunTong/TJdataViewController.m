...
@interface TJdataViewController ()<UITextFieldDelegate,UITextViewDelegate,UIActionSheetDelegate,ZYQAssetPickerControllerDelegate,UINavigationControllerDelegate,UICollectionViewDelegate,UICollectionViewDataSource>{
     NSData *imageData;
    NSString *fileName;
    ...
    NSString *ddkey;
    NSString *aaid;
    TJheaderCollectionReusableView *cellheader1;
...
@implementation TJdataViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title=@"添加工作日志";
}
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[TJheaderCollectionReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"TJheaderCollectionReusableView"];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionHeader) {
        ...
        cellheader1 = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"TJheaderCollectionReusableView" forIndexPath:indexPath];
        ...
        [TijiaoButton addTarget:self action:@selector(TijiaoButtonyes) forControlEvents:UIControlEventTouchUpInside];
...
-(void)intlaod{
    ...
    NSString *urls=[NSString stringWithFormat:@"http://www.eollse.cn:8080/grid/app/blog/uploadBlogImg.do"];
    NSDictionary *dics=@{
                         @"Cookie":strcookie,
...
-(void)TijiaoButtonyes{

//      NSString  *strend=[[NSString stringWithFormat:@"%@",_textFile.text] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
//    
   
    NSString *strarray = [[cellheader1.nsarray componentsJoinedByString:@","]stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    if (strarray.length!=0) {
        NSString *strurl=[NSString stringWithFormat:@"http://www.eollse.cn:8080/grid/app/blog/addOneBlog.do?blogName=%@&blogType=%@&blogContent=%@&blogPic=%@&@Cookie=%@",cellheader1.lablx.text,strarray,cellheader1.textView.text,hedimg,strcookie];
         NSString *url = [strurl stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
        [ZQLNetWork getWithUrlString:url success:^(id data) {
            NSLog(@"data========%@",data);
