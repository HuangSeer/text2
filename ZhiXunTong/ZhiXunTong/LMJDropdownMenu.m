...
- (void)setMenuTitles:(NSArray *)titlesArr rowHeight:(CGFloat)rowHeight{
    ...
    _listView.layer.borderWidth   = 2.0f;
...
- (void)showDropDown{   // 显示下拉列表
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _listView.frame  = CGRectMake(VIEW_X(_listView), VIEW_Y(_listView), VIEW_WIDTH(_listView),Screen_height/4.4);
...
- (void)hideDropDown{  // 隐藏下拉列表
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _tableView.frame = CGRectMake(0, -10, VIEW_WIDTH(_listView), VIEW_HEIGHT(_listView));
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_strpand containsString:@"level"]) {
        ...
        cell.textLabel.text =[NSString stringWithFormat:@"%@",loveM.level];
         return cell;
    }else if([_strpand containsString:@"OpinionClassId"]){
        lmmArray=[[NSMutableArray alloc]initWithArray:[SuQiuModel mj_objectArrayWithKeyValuesArray:_titleArr]];
        SuQiuModel *suqiu =[lmmArray objectAtIndex:indexPath.row];
        cell.textLabel.text =[NSString stringWithFormat:@"%@",suqiu.OpinionClassName];
        return cell;
    }else if([_strpand containsString:@"Deptid"]){
        lmmArray=[[NSMutableArray alloc]initWithArray:[SQbmModel mj_objectArrayWithKeyValuesArray:_titleArr]];
        SQbmModel *SQbm =[lmmArray objectAtIndex:indexPath.row];
        cell.textLabel.text =[NSString stringWithFormat:@"%@",SQbm.DeptName];
        return cell;
    
    }
    else if([_strpand containsString:@"Docket"]){
        lmmArray=[[NSMutableArray alloc]initWithArray:[ContentModel mj_objectArrayWithKeyValuesArray:_titleArr]];
        ContentModel *ContentM =[lmmArray objectAtIndex:indexPath.row];
        cell.textLabel.text =[NSString stringWithFormat:@"%@",ContentM.content];
        return cell;
        
    }
    else if([_strpand containsString:@"Secretary"]){
        lmmArray=[[NSMutableArray alloc]initWithArray:[SJtypeModel mj_objectArrayWithKeyValuesArray:_titleArr]];
        SJtypeModel *SJtypeM =[lmmArray objectAtIndex:indexPath.row];
        cell.textLabel.text =[NSString stringWithFormat:@"%@",SJtypeM.type];
        return cell;
        
    }
    else if([_strpand containsString:@"one"]){
        NSLog(@"_titleArr%@",_titleArr);
        lmmArray=[[NSMutableArray alloc]initWithArray:[LecwModel mj_objectArrayWithKeyValuesArray:_titleArr]];
        LecwModel *LecwM =[lmmArray objectAtIndex:indexPath.row];
        cell.textLabel.text =[NSString stringWithFormat:@"%@",LecwM.levelName];
        return cell;
    }
    else if([_strpand containsString:@"Nation"]){
        NSLog(@"_titleArr%@",_titleArr);
        lmmArray=[[NSMutableArray alloc]initWithArray:[NationModel mj_objectArrayWithKeyValuesArray:_titleArr]];
        ...
        lmModel *lMM =[lmmArray objectAtIndex:indexPath.row];
        cell.textLabel.text =[NSString stringWithFormat:@"%@",lMM.type];
        return cell;
    }
...
//     cell=[[_titleArr valueForKey:@"id"] objectAtIndex:indexPath.row];
    if ([_strpand containsString:@"level"]) {
    _LoveM=lmmArray[indexPath.row];
    
    }else if([_strpand containsString:@"Deptid"]){
           _suqiu=lmmArray[indexPath.row];
      
    }else if ([_strpand containsString:@"OpinionClassId"]){
    _SuQiuM=lmmArray[indexPath.row];
  
    }
    else if([_strpand containsString:@"Docket"]){
    _ContentM=lmmArray[indexPath.row];
    }
    else if([_strpand containsString:@"Secretary"]){
    _SJtypeM=lmmArray[indexPath.row];
    }
    else if([_strpand containsString:@"one"]){
        _LecwM=lmmArray[indexPath.row];
    }
    else if([_strpand containsString:@"Nation"]){
    ...
    _typeShop=lmmArray[indexPath.row];
    }
