...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _imgScrollView.showsVerticalScrollIndicator = YES;
...
- (void)nextImage{
    ...
    else if (_curIndex == self.pics.count) {
    ...
    CGFloat x = (_curIndex) * self.imgScrollView.frame.size.width;
    [UIView animateWithDuration:1.0 animations:^{
        _imgScrollView.contentOffset = CGPointMake(x, 0);
    } completion:^(BOOL finished) {
        if (finished && _curIndex == self.pics.count) {
...
- (void)addTimer{
    if (self.pics.count != 0) {
        self.time = [NSTimer scheduledTimerWithTimeInterval:10.0 target:self selector:@selector(nextImage) userInfo:nil repeats:YES];
    }
}
#pragma mark 下载图片
