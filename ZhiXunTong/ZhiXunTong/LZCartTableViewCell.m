...
//原价格
@property (nonatomic,retain) UILabel *yujialab;
//数量
...
- (void)awakeFromNib {
    [super awakeFromNib];
     self.nameLabel.numberOfLines = 0;//这句一定要写
    // Initialization code
}
...
//    self.lzImageView.image = model.image;
     [self.lzImageView sd_setImageWithURL:[NSURL URLWithString:model.image]placeholderImage:[UIImage imageNamed:@"默认图片"]];
    self.nameLabel.text = model.goodsName;
    self.priceLabel.text = model.price;
    self.priceLabel.text = model.price;
    self.numberLabel.text = [NSString stringWithFormat:@"%ld",(long)model.count];
    self.yujialab.text = [NSString stringWithFormat:@" ¥%@",model.goods_current_price];
...
-(void)setupMainView {
    ...
    CGFloat widths = (bgView.width - imageBgView.right)/1.5;
    //价格
    ...
    nameLabel.frame = CGRectMake(imageBgView.right, 10, widths, 25);
    ...
    dateLabel.frame = CGRectMake(nameLabel.left, nameLabel.bottom , width/1.5, 25);
    dateLabel.font = [UIFont systemFontOfSize:14];
    dateLabel.textColor = LZColorFromRGB(132, 132, 132);
    UIView *yuanx=[[UIView alloc]initWithFrame:CGRectMake(10, 12, dateLabel.frame.size.width-20, 1)];
    yuanx.backgroundColor=RGBColor(210, 210, 210);
    [dateLabel addSubview:yuanx];
    [bgView addSubview:dateLabel];
    self.yujialab = dateLabel;
