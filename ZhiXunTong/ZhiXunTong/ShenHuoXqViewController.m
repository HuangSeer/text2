...
#import "ShenHuoModel.h"
@interface ShenHuoXqViewController ()<UITableViewDelegate,UITableViewDataSource,UIWebViewDelegate>{
...
@implementation ShenHuoXqViewController
...
-(void)shujuTwo{
    ...
    [ZQLNetWork getWithUrlString:strurl success:^(id data) {
        NSLog(@"data==%@",data);
        _saveArray=[ShenHuoModel mj_objectArrayWithKeyValuesArray:[data valueForKey:@"Data"]];
...
-(void)BtnsClick:(UIButton *)sender
{
    if (sender.tag==50) {
        NSLog(@"111111");
        //  /api/APP1.0.aspx?method=PostThumb&TVInfoId=&Key=&Mid=&Uid=
        NSString *strurl=[NSString stringWithFormat:@"%@/api/APP1.0.aspx?method=PostThumb&TVInfoId=%@&Key=%@&Mid=%@&Uid=%@",URL,tvinfoId,key,_mid,aaid];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    {//评论
        ShenHuoModel *model=[_saveArray objectAtIndex:indexPath.row];
...
-(void)btn_dianZ:(UIButton *)sender{
    ShenHuoModel *model=[_saveArray objectAtIndex:sender.tag];
    NSLog(@"评论点赞：%@",model.id);
    //  /api/APP1.0.aspx?method=PostThumb&TVInfoId=&Key=&Mid=&Uid=
    NSString *strurl=[NSString stringWithFormat:@"%@/api/APP1.0.aspx?method=MThumb&TVInfoId=%@&Key=%@&Mid=%@&Uid=%@",URL,tvinfoId,key,model.id,aaid];
    //    NSString *urzm=[strurl stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSLog(@"%@",strurl);
    [ZQLNetWork getWithUrlString:strurl success:^(id data) {
        NSLog(@"data==%@",data);
